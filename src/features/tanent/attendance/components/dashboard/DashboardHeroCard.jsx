...
export const DashboardHeroCard = React.memo(({ today, access, actions }) => {
  ...
  const isBiometricDeviceOnly =
    access?.auto_synced ||
    (access?.available_methods?.length === 0 && access?.auto_synced);
  ...
  const getActionForStatus = () => {
    ...
    if (status === "PRESENT") return "CHECK_OUT";
    if (status === "ON_BREAK") return "BREAK_IN";
    return "CHECK_IN";
  };

  const getAltActionForStatus = () => {
    const status = today?.status;
    if (status === "PRESENT") return "BREAK_OUT";
    if (status === "ON_BREAK") return "CHECK_OUT";
    return null;
  };
  ...
  const handlePrimaryAction = () => {
    ...
    openModal(action, access);
  };

  const handleAltAction = () => {
    const action = getAltActionForStatus();
    if (action) {
      openModal(action, access);
    }
  };
  ...
  const renderAccessStatusDescriptor = () => {
    if (isBiometricDeviceOnly) {
      return (
        <div className="flex items-start gap-2.5 p-3.5 bg-slate-50 border border-slate-200 rounded-xl text-[11px] text-slate-500 font-normal leading-relaxed">
          ...
          <div>
            <span className="font-bold text-slate-800">Hardware Automation Synced:</span>{" "}
            Your attendance metrics compile automatically from physical biometric terminal logs.
            Web console interaction is read-only.
          </div>
        </div>
      );
    }
      ...
      if (faceStatus === "PENDING") {
        return (
          <div className="flex items-start gap-2.5 p-3.5 bg-amber-50/60 border border-amber-200/50 rounded-xl text-[11px] text-amber-800 font-normal leading-relaxed animate-fadeIn">
            ...
            <div>
              <span className="font-bold text-amber-900">Biometric Profile Pending Approval:</span>{" "}
              Your enrollment is currently undergoing administrative verification. Manual
              verification routes are blocked.
            </div>
          </div>
        );
      }
        ...
        return (
          <div className="flex items-start gap-2.5 p-3.5 bg-rose-50/60 border border-rose-200/50 rounded-xl text-[11px] text-rose-800 font-normal leading-relaxed animate-fadeIn">
            ...
            <div className="w-full flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
              <div>
                <span className="font-bold text-rose-900">Enrollment Matrix Rejected:</span>{" "}
                The facial structure vector did not pass internal consistency checks. Please update
                your data profile.
              </div>
              <button
                type="button"
      ...
      if (faceStatus === "NO_ENROLLMENT" && access?.face_required) {
        return (
          <div className="flex items-start gap-2.5 p-3.5 bg-indigo-50/60 border border-indigo-200/50 rounded-xl text-[11px] text-indigo-800 font-normal leading-relaxed animate-fadeIn">
            ...
            <div className="w-full flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
              <div>
                <span className="font-bold text-indigo-900">Biometric Signature Required:</span>{" "}
                Your tracking configuration mandates biometric verification profile registration
                before operations can clear.
              </div>
              <button
                type="button"
                onClick={() => navigate("/app/attendance/face-enrolment")}
    ...
    const isStrictAll =
      access?.validation_mode === "STRICT_ALL" ||
      access?.validation_mode === "ALL" ||
      access?.validation_mode === "FACE_GPS";
      ...
      <div className="space-y-2 pt-1">
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          {access?.available_methods?.map((m) => (
            <span
              key={m}
              className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-white border border-slate-200 text-slate-600 font-mono text-[9px] font-bold uppercase shadow-4xs"
            >
        ...
        <p className="text-[10px] text-slate-400 font-normal leading-normal">
          {isStrictAll ? (
            <span className="text-indigo-600 font-medium">
              ⚠️ Policy enforces strict alignment: Both location bounds checking AND identity
              validation must resolve simultaneously.
            </span>
          ) : (
            <span>
              Policy allows flexible execution routes. Clear checking can complete via any
              authenticated pipelines displayed above.
            </span>
          )}
        </p>
      </div>
  ...
  const getAltButtonLabel = () => {
    const status = today?.status;
    if (status === "PRESENT") return "Start Break";
    if (status === "ON_BREAK") return "Check Out";
    return null;
  };

  const altLabel = getAltButtonLabel();

  return (
    <>
      <div
        className={`w-full border rounded-2xl p-5 sm:p-6 transition-all duration-300 ring-4 ${statusConfig.colorClass}`}
      >
        <div className="flex flex-col space-y-4">
          <div className="flex justify-between items-start w-full">
            <div className="space-y-1">
              <span
                className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold tracking-wide uppercase border ${statusConfig.badgeClass}`}
              >
                {statusConfig.label}
              </span>
          ...
          {!isBiometricDeviceOnly && (
            <div className="pt-1 w-full max-w-xs space-y-2">
              ...
              {altLabel && !isButtonDisabled && (
                <button
                  type="button"
                  onClick={handleAltAction}
                  className="w-full py-2 px-4 bg-white border border-slate-200 text-slate-700 font-bold rounded-xl text-xs tracking-tight transition-all active:scale-98 cursor-pointer flex items-center justify-center gap-2 hover:bg-slate-50 focus:outline-hidden focus:ring-2 focus:ring-slate-200"
                >
                  <span>{altLabel}</span>
                </button>
              )}
      ...
      <CheckInModal
        ...
        actionLabel={actionLabels[actionType] || "Check In"}
